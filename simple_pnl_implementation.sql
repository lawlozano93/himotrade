-- Drop all functions and their dependent objects in one go using CASCADE
DROP FUNCTION IF EXISTS calculate_entry_fees(NUMERIC, NUMERIC) CASCADE;
DROP FUNCTION IF EXISTS calculate_total_fees(NUMERIC, NUMERIC) CASCADE;
DROP FUNCTION IF EXISTS calculate_unrealized_pnl(NUMERIC, NUMERIC, NUMERIC, TEXT) CASCADE;
DROP FUNCTION IF EXISTS calculate_true_pnl(NUMERIC, NUMERIC, NUMERIC, TEXT) CASCADE;
DROP FUNCTION IF EXISTS update_all_unrealized_pnl() CASCADE;
DROP FUNCTION IF EXISTS update_trade_entry_fees() CASCADE;
DROP FUNCTION IF EXISTS update_trade_info_display() CASCADE;
DROP FUNCTION IF EXISTS update_trade_unrealized_pnl() CASCADE;

-- Also explicitly drop the trigger and view just to be sure
